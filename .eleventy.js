...
eleventyConfig.addCollection("livePages", (collection) => {
  const isProd = process.env.ELEVENTY_ENV === "production" || process.env.NODE_ENV === "production";

  // load sitemap exclusions (safe if file missing)
  let excludePrefixes = [], excludeExact = [];
  try {
    const cfg = require(require("node:path").join(process.cwd(), "src/_data/sitemap.json"));
  ...
  return collection.getAll().filter((item) => {
    if (isProd && (item.data?.draft === true || item.data?.eleventyExcludeFromCollections)) return false;
  ...
  eleventyConfig.addCollection("llmsHybrid", (collectionApi) => {
    ...
    const curated = Array.from(groupedCuratedMap.entries()).map(([title, items]) => ({ title, items }));
