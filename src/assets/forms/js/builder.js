(function () {
      ...
      .then(function (res) {
        ...
        if (ctype.indexOf('application/json') === -1) {
          return res.text().then(function (t) {
            if (t && /^\s*</.test(t)) {
              throw new Error(
                'Non-JSON response for ' + url + ' (likely HTML). First chars: ' + t.slice(0, 160)
              );
            }
            // try to parse anyway
            try {
  ...
  function sanitizeItem(it, fallbackId, catId) {
    if (!it || typeof it !== 'object')
      return { id: fallbackId, item: String(it || ''), category_id: catId || '' };
    return {
      id: it.id || fallbackId,
  ...
  VC.normalize = function (list) {
    ...
    if (list && Array.isArray(list.sections)) {
      ...
      for (var i = 0; i < list.sections.length; i++) {
        ...
        cidNorm = VC.slug(sec.title || 'section-' + (i + 1));
        var items = [];
        var src = sec.items || [];
        for (var j = 0; j < src.length; j++) {
          ...
          items.push(
            typeof it === 'string'
              ? { id: VC.slug(sec.title) + '-' + (j + 1), item: it, category_id: cidNorm }
              : sanitizeItem(it, VC.slug(sec.title) + '-' + (j + 1), cidNorm)
          );
        }
        secs.push({
          title: sec.title || 'Section',
          note: sec.note || sec.subtitle || '',
          items: items,
          cidNorm: cidNorm,
        });
      }
    ...
    if (list && Array.isArray(list.categories) && Array.isArray(list.items)) {
      ...
      for (var k = 0; k < list.items.length; k++) {
        ...
        cidNorm = itm.category_id || 'uncategorized';
        if (!grouped[cidNorm]) grouped[cidNorm] = [];
        grouped[cidNorm].push(
          sanitizeItem(itm, cidNorm + '-' + Math.random().toString(36).slice(2, 8), cidNorm)
        );
      }
      var outSecs = [];
    ...
    // items only
    var itemsOnly = [],
      srcItems = (list && list.items) || [];
    for (var m = 0; m < srcItems.length; m++) {
      cidNorm = 'items';
      itemsOnly.push(
        typeof srcItems[m] === 'string'
          ? { id: 'item-' + (m + 1), item: srcItems[m], category_id: cidNorm }
          : sanitizeItem(srcItems[m], 'item-' + (m + 1), cidNorm)
      );
    }
    return {
      title: (list && list.title) || 'Checklist',
      description: (list && list.description) || '',
      sections: [{ title: 'Items', items: itemsOnly, cidNorm: 'items' }],
    };
  };
  ...
  function renderChecklistDescription(desc) {
    ...
    if (!parent) return;
    if (!host) {
      host = VC.el('div', 'mt-3 text-muted', '');
      host.id = hostId;
  ...
  VC.render = function (norm) {
    VC.state.norm = norm;
    var cont = VC.els.container;
    if (!cont) {
      VC.dwarn('Missing container');
      return;
    }
    cont.innerHTML = '';
    ...
    for (var i = 0; i < norm.sections.length; i++) {
      ...
      if (sec.cidRend) wrap.setAttribute('data-cid', sec.cidRend);
      ...
      (function (b, l) {
        b.addEventListener('click', function () {
          ...
          if (isCol) {
            ...
            b.setAttribute('data-collapsed', '0');
          } else {
      ...
      for (var j = 0; j < (sec.items || []).length; j++)
        list.appendChild(buildItem(sec.items[j], sec.cidRend));
  ...
  function buildItem(node, sectionCid) {
    ...
    var body = VC.el('div', 'card-body p-3');
    ...
    if (isObj) {
      ...
      if (node.joint_roles && node.joint_roles.length)
        card.setAttribute('data-joint-roles', node.joint_roles.join(', '));
    ...
    var evid =
      isObj && node.evidence_type && node.evidence_type.length
        ? VC.canonEvidenceKeys(node.evidence_type)
        : [];
    if (evid.length) card.setAttribute('data-evidence', evid.join(', '));

    var title = VC.el('div', 'fw-semibold mb-2', text);
    ...
    var statusLbl = VC.el('div', 'text-muted small mb-1', 'Status *');
    statusCol.appendChild(statusLbl);

    var sel = VC.el('select', 'form-select form-select-sm status-select');
    for (var k = 0; k < VC.STATUS.length; k++) {
      var o = VC.el('option', null, VC.STATUS[k].t);
      o.value = VC.STATUS[k].v;
      sel.appendChild(o);
    }
    statusCol.appendChild(sel);

    var notesCol = VC.el('div', 'notes-col flex-grow-1 d-flex flex-column');
    var notesLbl = VC.el('div', 'text-muted small mb-1', 'Notes');
    notesCol.appendChild(notesLbl);
    var notes = VC.el('textarea', 'form-control form-control-sm notes');
    notes.rows = 2;
    notes.placeholder = 'Add notes (optional)';
    ...
    if (evid.length || VC.config.ALLOW_ATTACH || VC.state.view === 'client') {
      ...
      if (VC.config.ALLOW_ATTACH || VC.state.view === 'client') {
        ...
        var itemId =
          card.getAttribute('data-item-id') || cid + '-' + Math.random().toString(36).slice(2, 8);
      ...
      if (evid.length) {
        ...
        for (var e = 0; e < evid.length; e++) {
          badgesWrap.appendChild(
            VC.el('span', 'badge bg-light text-secondary border', VC.evidenceLabel(evid[e]))
          );
        }
  ...
  VC.harvest = function () {
    var out = {
      ...
      reviewer:
        VC.els.reviewer && VC.els.reviewer.value ? String(VC.els.reviewer.value).trim() : '',
    ...
    for (var i = 0; i < secEls.length; i++) {
      ...
      for (var j = 0; j < cards.length; j++) {
        ...
        sec.items.push({
          ...
          responsibility:
            card.getAttribute('data-responsibility') ||
            (card.getAttribute('data-joint-roles') ? 'Joint' : ''),
          joint_roles: (card.getAttribute('data-joint-roles') || '')
            .split(',')
            .map(function (s) {
              return s.trim();
            })
  ...
  VC.loadChecklist = function (file) {
    ...
    fetchJson(url, function (err, raw) {
      if (err) {
        VC.derr('checklist load error', err);
        alert('Failed to load checklist: ' + err.message);
        return;
      }
      VC.state.raw = raw;
