...
(function () {
  ...
  try {
    ...
    if (state?.debug) console.error('Checklist debug initialization failed:', e);
  }

  function dlog() {
    if (state.debug && window.console && console.log)
      console.log.apply(console, ['[Checklist]'].concat([].slice.call(arguments)));
  }
  function dwarn() {
    if (state.debug && window.console && console.warn)
      console.warn.apply(console, ['[Checklist]'].concat([].slice.call(arguments)));
  }
  function derr() {
    if (state.debug && window.console && console.error)
      console.error.apply(console, ['[Checklist]'].concat([].slice.call(arguments)));
  }
  ...
  function fetchJson(url, cb) {
    ...
    fetch(url, { cache: 'no-store' })
      ...
      .then(function (json) {
        dlog('fetch keys', Object.keys(json || {}));
        cb(null, json);
      })
      .catch(function (err) {
  ...
  function normalize(list) {
    ...
    if (list && Array.isArray(list.sections)) {
      ...
      for (var i = 0; i < list.sections.length; i++) {
        ...
        for (var j = 0; j < srcItems.length; j++) {
          ...
          items.push(
            typeof it === 'string'
              ? { id: slug(sec.title) + '-' + (j + 1), item: it }
              : sanitizeItem(it, slug(sec.title) + '-' + (j + 1))
          );
        }
        secs.push({
          title: sec.title || 'Section',
          note: sec.note || sec.subtitle || list.description || '',
          items: items,
        });
      }
    ...
    if (list && Array.isArray(list.categories) && Array.isArray(list.items)) {
      ...
      var byId = {};
      for (var c = 0; c < list.categories.length; c++)
        byId[list.categories[c].id] = list.categories[c];
      var grouped = {};
      ...
      for (var key in grouped) {
        ...
        outSecs.push({
          title: byId[key] && byId[key].name ? byId[key].name : 'Section',
    ...
    for (var m = 0; m < src.length; m++) {
      ...
      itemsOnly.push(
        typeof ii === 'string'
          ? { id: 'item-' + (m + 1), item: ii }
          : sanitizeItem(ii, 'item-' + (m + 1))
      );
    }
    return {
      title: (list && list.title) || 'Checklist',
      sections: [{ title: 'Items', items: itemsOnly }],
    };
  }
  ...
  function render(norm) {
    state.norm = norm;
    if (!els.container) {
      dwarn('Missing container element');
      return;
    }
  ...
  function buildItem(node, idx) {
    ...
    if (isObj) {
      ...
      if (node.joint_roles && node.joint_roles.length)
        card.dataset.jointRoles = node.joint_roles.join(', ');
      if (node.id) card.dataset.itemId = node.id;
      if (node.evidence_type && node.evidence_type.length)
        card.dataset.evidence = node.evidence_type.join(', ');
      if (node.frequency) card.dataset.frequency = node.frequency;
    }

    var title = el('div', 'fw-semibold mb-2', text);
  ...
  function harvest() {
    ...
    for (var i = 0; i < secEls.length; i++) {
      ...
      for (var j = 0; j < cards.length; j++) {
        var card = cards[j];
        var q =
          (card.querySelector('.fw-semibold') && card.querySelector('.fw-semibold').textContent) ||
          '';
        var status =
          (card.querySelector('.status-select') && card.querySelector('.status-select').value) ||
          '';
        ...
        sec.items.push({
          ...
          joint_roles: (card.dataset.jointRoles || '')
            .split(',')
            .map(function (s) {
              return s.trim();
            })
  ...
  function collectCitations(d) {
    ...
    for (var i = 0; i < d.sections.length; i++) {
      ...
      for (var j = 0; j < sec.items.length; j++) {
        var it = sec.items[j];
        if (it.citation)
          arr.push({ section: sec.title, question: it.question, citation: it.citation });
      }
    }
    return arr;
  }
  ...
  function safeName(s) {
    return String(s || 'report').replace(/[^a-z0-9-_]+/gi, '_');
  }
  ...
  function exportJSON() {
    ...
    downloadBlob(
      ...
      safeName(d.title) + '_' + d.date + '_report.json'
    );
  }
  ...
  function exportTXT() {
    ...
    if (cites.length) {
      ...
      for (var k = 0; k < cites.length; k++)
        lines.push('- ' + cites[k].citation + ' - ' + cites[k].question);
    }
    downloadBlob(
      new Blob([lines.join('\n')], { type: 'text/plain' }),
      safeName(d.title) + '_' + d.date + '_report.txt'
    );
  }
  ...
  function exportPDF() {
    ...
    if (!jsPDF) {
      alert('jsPDF not found - exporting JSON instead.');
      return exportJSON();
    }
    ...
    function ensure(space) {
      if (y + space > pageH - margin.y) addPage();
    }

    doc.setFont('helvetica', 'bold');
    doc.setFontSize(16);
    doc.text(d.title, margin.x, y);
    y += 20;
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(10);
    doc.text('Client: ' + d.client, margin.x, y);
    y += 14;
    doc.text('Reviewer: ' + d.reviewer, margin.x, y);
    y += 14;
    doc.text('Date: ' + d.date, margin.x, y);
    y += 18;
    ...
    for (var i = 0; i < d.sections.length; i++) {
      ...
      for (var j = 0; j < sec.items.length; j++) {
        ...
        for (var a = 0; a < wrapped.length; a++) {
          doc.text(wrapped[a], margin.x, y);
          y += 12;
        }
        if (it.status) {
          doc.text('Status: ' + it.status, margin.x + 14, y);
          y += 12;
        }
        if (it.notes) {
          var wn = doc.splitTextToSize('Notes: ' + it.notes, pageW - margin.x * 2 - 14);
          for (var b = 0; b < wn.length; b++) {
            doc.text(wn[b], margin.x + 14, y);
            y += 12;
          }
        }
        y += 6;
    ...
    if (cites.length) {
      ...
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(14);
      doc.text('Appendix - Citations', margin.x, y);
      y += 20;
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(10);
  ...
  function validate() {
    ...
    for (var j = 0; j < selects.length; j++) {
      var sel = selects[j];
      if (!sel.value) {
        if (sel && sel.classList) sel.classList.add('is-invalid');
        ok = false;
        if (!first) first = sel;
      } else if (sel && sel.classList) sel.classList.remove('is-invalid');
    }
    if (!ok && first && first.scrollIntoView) {
  ...
  function loadManifest() {
    ...
    fetchJson(url, function (err, manifest) {
      if (err) {
        derr('manifest error', err);
        alert('Failed to load checklist index.');
        return;
      }
      state.manifest = manifest;
      var lists =
        (manifest && (manifest.lists || manifest.checklists)) ||
        (Array.isArray(manifest) ? manifest : []);
      dlog('manifest lists', lists.length, lists);

      if (!els.type) {
        dwarn('Dropdown element not found');
        return;
      }
      els.type.innerHTML = '';
      ...
      var last = null;
      try {
        last = localStorage.getItem('vulpine.checklist.lastFile');
      } catch (e) {
        if (state?.debug) console.error('Checklist debug initialization failed:', e);
      }
      if (last) {
        for (var j = 0; j < els.type.options.length; j++)
          if (els.type.options[j].value === last) els.type.selectedIndex = j;
      }

      var initial = els.type.value || (lists[0] && lists[0].file) || '';
      if (initial) {
        dlog('initial load', initial);
        loadChecklist(initial);
      }

      els.type.addEventListener('change', function (e) {
  ...
  function loadChecklist(file) {
    ...
    try {
      localStorage.setItem('vulpine.checklist.lastFile', file);
    } catch (e) {
      if (state?.debug) console.error('Checklist debug initialization failed:', e);
    }
  ...
  function mountToggleIfMissing() {
    if (document.getElementById('toggleCitations')) {
      dlog('toggle exists');
      return;
    }
    if (!els.type) {
      dwarn('toggle mount skipped - no dropdown');
      return;
    }
    var label = el('label', 'form-check ms-2');
    var input = el('input', 'form-check-input');
    input.type = 'checkbox';
    input.id = 'toggleCitations';
    ...
    input.addEventListener('change', function () {
      if (state.norm) render(state.norm);
    });
    dlog('toggle mounted');
  }
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    window.VulpineChecklist = {
      ...
      setDebug: function (v) {
        state.debug = !!v;
        try {
          localStorage.setItem('vulpine.checklist.debug', state.debug ? '1' : '0');
        } catch (e) {
          if (state?.debug) console.error('Checklist debug initialization failed:', e);
        }
        dlog('debug set', state.debug);
      },
    };
