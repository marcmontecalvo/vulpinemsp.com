(function () {
  ...
  function makeSentences(n) {
    var s = [],
      i;
    for (i = 0; i < n; i++) s.push(pick(LOREM));
    return s.join(' ');
  }
  function makeParagraph(a, b) {
    return makeSentences(randInt(a, b));
  }
  ...
  function generateNote(kind) {
    ...
    if (r < 0.8) return generateNote('short');
  ...
  VC.quickFillHeaderNames = function (opts) {
    fillHeaderNames(opts);
  };
