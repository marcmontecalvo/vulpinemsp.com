(function () {
  ...
  function optionize(sel, arr, labelAll) {
    ...
    sel.innerHTML = '';
    var optAll = VC.el('option', null, labelAll || 'All');
    optAll.value = '';
    ...
    for (var i = 0; i < arr.length; i++) {
      var row = arr[i],
        text,
        val;
      if (row && typeof row === 'object' && ('v' in row || 't' in row)) {
        val = row.v || row.t || '';
        text = row.t || row.v || '';
      } else {
        val = String(row);
  ...
  function ensureBar() {
    if (F.els.bar) return;
    var host = VC.el('div', 'mt-3');
    host.id = 'vcFilterBar';
    var row = VC.el('div', 'row g-2 align-items-end');
    host.appendChild(row);

    var cCol = VC.el('div', 'col-12 col-md-4');
    var cLbl = VC.el('label', 'form-label', 'Filter - Category');
    cLbl.htmlFor = 'vcFilterCat';
    var cSel = VC.el('select', 'form-select');
    cSel.id = 'vcFilterCat';
    ...
    var rLbl = VC.el('label', 'form-label', 'Filter - Responsibility');
    rLbl.htmlFor = 'vcFilterResp';
    var rSel = VC.el('select', 'form-select');
    rSel.id = 'vcFilterResp';
    rCol.appendChild(rLbl);
    rCol.appendChild(rSel);
    row.appendChild(rCol);

    var eCol = VC.el('div', 'col-12 col-md-4');
    var eLbl = VC.el('label', 'form-label', 'Filter - Evidence Type');
    eLbl.htmlFor = 'vcFilterEvid';
    var eSel = VC.el('select', 'form-select');
    eSel.id = 'vcFilterEvid';
    eCol.appendChild(eLbl);
    eCol.appendChild(eSel);
    ...
    F.els.resp = rSel;
    F.els.evid = eSel;
    F.els.clear = clearBtn;
    F.els.collapseAll = collapseBtn;
    F.els.expandAll = expandBtn;
    ...
    collapseBtn.addEventListener('click', function (e) {
      e.preventDefault();
      if (VC.collapseAllSections) VC.collapseAllSections();
    });
    expandBtn.addEventListener('click', function (e) {
      e.preventDefault();
      if (VC.expandAllSections) VC.expandAllSections();
    });
  }
  ...
  function categoriesFromDom() {
    ...
    for (var i = 0; i < secs.length; i++) {
      ...
      if (!Object.prototype.hasOwnProperty.call(seen, cid)) {
        seen[cid] = 1;
        out.push({ v: cid, t: title });
      }
    }
  ...
  function tagDomCategories() {
    ...
    for (var i = 0; i < secs.length; i++) {
      ...
      for (var j = 0; j < cards.length; j++) {
        if (!cards[j].getAttribute('data-category-id'))
          cards[j].setAttribute('data-category-id', cid);
      }
    }
  }
  ...
  function applyFilters() {
    ...
    for (var s = 0; s < sections.length; s++) {
      ...
      if (!section || !section.querySelector) continue;
  ...
  function recomputeOptions() {
    ...
    for (var i = 0; i < cards.length; i++) {
      ...
      var resp =
        c.getAttribute('data-responsibility') ||
        (c.getAttribute('data-joint-roles') ? 'Joint' : '');
      var evs = (c.getAttribute('data-evidence') || '')
        .split(',')
        .map(function (s) {
          return (s || '').trim();
        })
    ...
    var catsArr = categoriesFromDom().filter(function (x) {
      return Object.prototype.hasOwnProperty.call(cats, x.v);
    });
    var respsArr = uniqueStrings(Object.keys(resps));
    var evidArr = uniqueStrings(Object.keys(evids)).map(function (k) {
      return { v: k, t: VC.evidenceLabel(k) };
    });
