(function () {
  ...
  // --- Debug logging ---
  VC.dlog = function () {
    if (VC.config.DEBUG && console && console.log) console.log.apply(console, arguments);
  };
  VC.derr = function () {
  ...
  VC.applyStatusUi = function (sel) {
    ...
    switch (v) {
      case 'yes':
        sel.classList.add('status-yes');
        break;
      case 'no':
        sel.classList.add('status-no');
        break;
      case 'needs':
        sel.classList.add('status-needs');
        break;
      case 'na':
        sel.classList.add('status-na');
        break;
      case 'unknown':
  ...
  VC.canonEvidenceKeys = function (arr) {
    var out = {},
      ...
      k,
      idx = {};
    for (k in VC.EVIDENCE_CANON_MAP)
      if (Object.prototype.hasOwnProperty.call(VC.EVIDENCE_CANON_MAP, k)) {
        var list = VC.EVIDENCE_CANON_MAP[k];
        for (i = 0; i < list.length; i++) idx[list[i]] = k;
      }
  ...
  VC.evidenceLabel = function (key) {
    ...
    return k.replace(/_/g, ' ').replace(/\b[a-z]/g, function (c) {
